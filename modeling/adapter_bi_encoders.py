...
class EntityLinker(nn.Module):
    ...
    def __init__(self, adapter=False, bitfit=False, lora=False, r=64):
        ...
        self.mention_encoder = AutoModel.from_pretrained('bert-base-uncased')
        ...
        if adapter:
            self.mention_encoder.add_adapter("mention_adapter", config='pfeiffer')
            self.mention_encoder.train_adapter("mention_adapter")

            self.entity_encoder.add_adapter("entity_adapter", config='pfeiffer')
            self.entity_encoder.train_adapter("entity_adapter")

        if bitfit:
            _deactivate_relevant_gradients(self.mention_encoder, ['bias'])
            _deactivate_relevant_gradients(self.entity_encoder, ['bias'])

        if lora:
            config = LoRAConfig(r=r, use_gating=False)     # Wx + alpha / r * BAx r = 1, 8, 64
            self.mention_encoder.add_adapter("mention_lora_adapter", config=config)
            self.mention_encoder.train_adapter("mention_lora_adapter")
            self.entity_encoder.add_adapter("entity_lora_adapter", config=config)
            self.entity_encoder.train_adapter("entity_lora_adapter")

        total_num_parameters = 0
        for hr_param, tail_param in zip(self.mention_encoder.named_parameters(), self.entity_encoder.named_parameters()):
            total_num_parameters += hr_param[1].numel() if hr_param[1].requires_grad else 0
            total_num_parameters += tail_param[1].numel() if tail_param[1].requires_grad else 0
