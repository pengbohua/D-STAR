...
class Trainer:
    ...
    def __init__(self,
        ...
        elif model_type == 'prefix_contrastive':
            from modeling.prefix_bi_encoders import PrefixEntityLinker
            self.model = PrefixEntityLinker(prefix_seq_len=prefix_length)
        elif model_type == 'contrastive_colbert':
            from modeling.colbert import EntityLinker
            self.model = EntityLinker()
        else:
    ...
    def evaluate(self, step=0):
        if not self.is_training:
            ...
            if "prefix" in self.model_type:
                state_dict = {key: value for (key, value) in self.model.state_dict().items() if
                                  "prefix_encoder" in key or "linear" in key}
