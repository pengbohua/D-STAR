...
from utility.utils.hit_scores import compute_hit_scores
...
def main(dataset, split, data_dir, collection, rankings, k_values):
    ...
    #### Provide the data_dir where nfcorpus has been downloaded and unzipped
    corpus, queries, qrels = GenericDataLoader(data_folder=data_dir, query_file="test_queries.jsonl").load(split=split)

    inv_map, results = {}, {}
    
    #### Document mappings (from  position in tsv file to original string file idx） ####
    ...
    for _, row in tsv_reader(rankings):
        ...
        if qid != inv_map[str(doc_id)]:
            if qid not in results:
                results[qid] = {inv_map[str(doc_id)]: 1 / rank}     # rank is one based
            else:
                results[qid][inv_map[str(doc_id)]] = 1 / rank

    metrics = compute_hit_scores(qrels, results)
    output_path = "/".join(rankings.split('/')[:-1])

    #### Evaluate your retrieval using NDCG@k, MAP@K ... P@K
    evaluator = EvaluateRetrieval()
    ndcg, _map, recall, precision = evaluator.evaluate(qrels, results, k_values)    
    # mrr = EvaluateRetrieval.evaluate_custom(qrels, results, k_values, metric='mrr')
    
    # metrics.update({"MRR": mrr})
    with open(osj(output_path, 'hits.json'), 'w') as f:
        json.dump(metrics, f)
    print(metrics)
    exit()
