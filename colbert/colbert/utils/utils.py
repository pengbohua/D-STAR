...
def load_checkpoint(path, model, optimizer=None, do_print=True):
    ...
    checkpoint['model_state_dict'].update({key: value for (key, value) in new_state_dict.items() if "prefix_encoder" in key or "linear" in key})
