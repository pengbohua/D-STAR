...
from colbert.indexing.faiss_index import FaissIndex, FaissFlatIndex
...
def get_flat_index_name(offset=None, endpos=None):
    range_info = '' if offset is None else f'.{offset}-{endpos}'

    return f'flat{range_info}.faiss'
...
def prepare_faiss_index(slice_samples_paths, partitions, sample_fraction=None):
    ...
    # index = FaissFlatIndex(dim, partitions)
...
def index_faiss(args):
    ...
    for slice_idx, part_offset in enumerate(range(0, len(parts), num_parts_per_slice)):
        ...
        # approximate knn, building cluster centroids from  sample data.
...
def index_flat_faiss(args):
    ...
    for slice_idx, part_offset in enumerate(range(0, len(parts), num_parts_per_slice)):
        ...
        if args.slices == 1:
            faiss_index_name = get_flat_index_name()
        else:
            faiss_index_name = get_flat_index_name(offset=part_offset, endpos=part_endpos)
        ...
        thread = threading.Thread(target=_loader_thread, args=(slice_parts_paths,))     # multithread embs loading
        thread.start()
        ...
        for filenames in grouper(slice_parts_paths, SPAN, fillvalue=None):
            ...
            print_message("#> Processing a sub_collection with shape", sub_collection.shape)
            index.add(sub_collection)
            index_offset += sub_collection.shape[0]
    ...
    print('total index length: {}'.format(index_offset))
