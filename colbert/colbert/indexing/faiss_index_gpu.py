...
class FaissIndexGPU():
    ...
    def _make_vres_vdev(self):
        ...
        vres = faiss.GpuResourcesVector()       # empty vector to manage gpu
        vdev = faiss.IntVector()    # creates an empty vector of integers to store gpus device IDs
    ...
    def add(self, index, data, offset):
        # work similar with the original add method in the cpu setting
        assert self.ngpu > 0
        ...
        for i0 in range(0, nb, self.add_batch_size):
            ...
            self.gpu_index.add_with_ids(xs, np.arange(offset+i0, offset+i1))        # add gpu indexes with ids
